...
//PDO在根的存在，所以你需要一个反斜杠您常量的开始 new \PDO 或者直接导入命名空间
use PDO;
use PDOException;
use Exception;
...
class Mysql
{
    ...
    public function configMaster($host, $database, $user, $password, $port = null, $driver = 'mysql')
    {
    ...
    public function configSlave($host, $database, $user, $password, $port = null, $driver = 'mysql')
    {
    ...
    protected function getSlave()
    {
        if (!isset($this->pdo_slave)) {
            $this->pdo_slave = $this->createConnection(
                    $this->config_slave['driver'],
                    $this->config_slave['host'],
                    $this->config_slave['database'],
                    $this->config_slave['user'],
                    $this->config_slave['password'],
                    $this->config_slave['port']
            );
        }
